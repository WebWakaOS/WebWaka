...
Outstanding S05 items: (1) LGA chairs (774 LGAs) — no consolidated national source, requires 36 per-state SIEC batches; (2) state assembly members for 35 non-Lagos states — requires per-state official directory batches; (3) constituency and ward-rep offices — no machine-readable consolidated source. Ward representatives and campaign offices are deferred indefinitely. Full deferred-items research is at `docs/reports/phase-s05-deferred-items-source-research-2026-04-22.md`. .bak files (0306 polling units 168MB, 0307 NEMIS schools 661MB, 0308 NHIA HCP 23MB, 0309 NPHCDA PHC 132MB) pending chunked apply to both environments.

D1 remote deploy: ALL migrations 0301–0314b APPLIED to staging (52719457-5d5b-4f36-9a13-c90195ec78d2) and production (72fa5ec8-52c2-4f41-b486-957d7b00c76f) on 2026-04-22. Production final counts: 9,255 individuals, 9,255 politician_profiles, 11,086 places, 11,080 jurisdictions, 9,306 seed_ingestion_records, 11 seed_runs. Fix log: (1) `seed_sources.freshness_status='historical'` invalid per CHECK — changed to 'superseded' in 0314; (2) 0314b lacked seed_run INSERT for `_part2` ID — prepended; (3) production required manual geography bootstrap (country/zones/states/LGAs/wards/jurisdictions from infra/db/seed/) before migrations; (4) D1 multi-stmt batch `D1_RESET_DO` errors resolved by applying oversized stmts individually.
