# Pillar 2 Template Agent Handoff Protocol

**Document type:** Mandatory agent workflow — Pillar 2 template creation  
**Status:** ACTIVE  
**Date:** 2026-04-25  
...
> to research and implement a Pillar 2 niche website template for WebWaka OS.  
...
- `docs/reports/pillar2-niche-identity-system-2026-04-25.md` — this system's full design report
- `docs/reports/pillar2-forensics-report-2026-04-24.md` — current Pillar 2 runtime reality

**Template system (mandatory):**
- `docs/templates/pillar2-niche-registry.schema.md` — registry field definitions
- `docs/templates/pillar2-template-status-codes.md` — status transition rules
...
**Registry (mandatory):**
- `docs/templates/pillar2-niche-registry.json` — the canonical niche list and statuses
- `docs/templates/pillar2-template-queue.md` — the next prioritised niche
...
```
ACTION: Read docs/templates/pillar2-niche-registry.json
...
Also read:
  ...
  - The vertical's row in the Pillar 2 niche registry for audienceSummary, businessContextSummary, contentLocalizationNotes
```
...
Find: How do Nigerians contact/book/engage with this type of business? (WhatsApp? Phone? Walk-in?)
...
/**
 * {NicheName} — Pillar 2 Website Template
...
ACTION: Update docs/templates/pillar2-template-queue.md
  - Move this niche from CURRENT to COMPLETED
  - Advance NEXT to the next READY_FOR_IMPLEMENTATION niche
...
5. **Class naming** — use `.ww-*` prefix for all design system classes (`.ww-btn`, `.ww-container`, etc.)
6. **Accessibility** — all interactive elements need `aria-label` where not self-describing; skip links present in base
7. **Semantic HTML** — `<header>`, `<main>`, `<section>`, `<footer>`, `<nav>` required structure

### Performance Standards

1. **No inline images** — reference image URLs from `ctx.logoUrl` or external CDN; no base64 data URIs
...
4. **Hero images** — use CSS background-image with `background-size: cover` rather than `<img>` for hero sections where possible

### Content Standards

1. **Nigerian English** — use "organisation" not "organization"; "colour" not "color" in user-visible text
...
- [ ] CTA uses direct Nigerian conversion language (e.g., "Book Now", "Order Today via WhatsApp", "Call Us")
- [ ] Testimonials reference Nigerian context (names, locations, scenarios)
- [ ] "About" section uses trust language relevant to Nigeria (e.g., years in business, local area served, certifications)
- [ ] No American spelling defaults (check: organize vs organise, color vs colour, etc.)
- [ ] No generic taglines detached from Nigerian reality

### Geographic References
- [ ] Example city/neighbourhood references are Nigerian (Lagos / Abuja / Kano / PH / Ibadan / Owerri / Enugu — pick relevant)
- [ ] No London, New York, or other western city references
- [ ] LGA-level specificity used where appropriate (e.g., "serving Surulere and Yaba")

### Payment & Pricing
- [ ] All example prices in NGN / Kobo (₦5,000 not $5)
- [ ] Payment methods mentioned are Nigerian (Paystack, bank transfer, USSD, POS cash)
- [ ] No Stripe, PayPal, Venmo, or western payment references
...
### Trust & Regulatory
- [ ] Applicable Nigerian certifications noted (NAFDAC, CAC reg no., NBA membership, NMA licence, etc.)
- [ ] Nigerian professional body affiliations shown where relevant
- [ ] "Verified by" or "Licensed by" trust badges included where applicable

### Visual Direction
- [ ] Image descriptions specify Nigerian/African subjects (not western stock photo defaults)
- [ ] Colour choices respect brand tokens (--ww-primary, --ww-secondary) — no hardcoded off-brand colours
- [ ] No generic western lifestyle imagery direction
- [ ] For food verticals: Nigerian cuisine imagery specified
- [ ] For professional verticals: Nigerian professionals in Nigerian office/studio settings
...
### Language
- [ ] Template supports Pidgin English awareness (copy can be adapted by tenant)
- [ ] No language that assumes English-first Nigerian audience (Yoruba / Igbo / Hausa names used in examples)
...
Set owner = null (release the claim so another agent can pick it up later)
Document in notes field
```
...
Read the queue file for the next available niche.
...
docs/templates/pillar2-niche-registry.json                                ← updated status
docs/templates/pillar2-template-execution-board.md                        ← updated board
docs/templates/pillar2-template-queue.md                                  ← advanced queue
```
