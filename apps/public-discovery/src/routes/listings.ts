...
router.get('/in/:placeId', async (c) => {
  ...
  return c.html(baseTemplate({ title: `Businesses in ${placeName}`, body, headExtra }));
});
...
router.get('/search', async (c) => {
  ...
  return c.html(baseTemplate({ title: `"${q}" — Search`, body, headExtra }));
});
...
router.get('/category/:cat', async (c) => {
  ...
  return c.html(baseTemplate({
    ...
    structuredData: {
      ...
      url: `https://discover.webwaka.ng/discover/category/${cat}`,
    },
  }));
});
