/**
 * Listings in a geography — /discover/in/:placeId?page=…
 * D1-4: geography filter + pagination
 */
...
export default function InPlace() {
  ...
  const load = useCallback(() => {
    ...
    discoveryApi.inPlace(placeId, page)
      .then(d => {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem 1.25rem' }}>
      <nav style={{ marginBottom: '1rem', fontSize: '0.8125rem', color: 'var(--muted)' }}>
        <Link to="/discover">Home</Link> &rsaquo; {place?.name ?? 'Businesses'}
      </nav>

      <h1 style={{ fontWeight: 700, fontSize: '1.375rem', marginBottom: '1.5rem' }}>
        Businesses in {place?.name ?? '…'}
        {total > 0 && (
      ...
      {!loading && listings.length === 0 && !error && (
        <div style={{ textAlign: 'center', padding: '3rem 0', color: 'var(--muted)' }}>
          <p style={{ fontSize: '1.125rem', marginBottom: '0.5rem' }}>No listings yet for {place?.name}.</p>
