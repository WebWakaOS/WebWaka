...
describe('SuperAgent Routes — M12 Integration', () => {
  ...
  // -------------------------------------------------------------------------
  // MON-03: GET /superagent/usage/quota
  // -------------------------------------------------------------------------

  describe('MON-03: GET /superagent/usage/quota', () => {
    function makeQuotaDB(opts: {
      walletRow?: { current_month_spent_wc: number; spend_cap_monthly_wc: number; spend_cap_reset_at: string } | null;
      subscriptionPlan?: string | null;
      planQuotaRow?: { quota_waku_cu: number; description: string } | null;
    } = {}) {
      return {
        prepare: vi.fn().mockImplementation((sql: string) => {
          const lc = sql.toLowerCase();
          if (lc.includes('wc_wallets')) {
            const row = opts.walletRow !== undefined
              ? opts.walletRow
              : { current_month_spent_wc: 250, spend_cap_monthly_wc: 500, spend_cap_reset_at: '2026-05-01' };
            return { bind: () => ({ first: async <T>() => row as T, all: async () => ({ results: [] }), run: async () => ({ success: true }) }) };
          }
          if (lc.includes('subscriptions')) {
            const plan = opts.subscriptionPlan !== undefined ? opts.subscriptionPlan : 'free';
            const row = plan ? { plan } : null;
            return { bind: () => ({ first: async <T>() => row as T, all: async () => ({ results: [] }), run: async () => ({ success: true }) }) };
          }
          if (lc.includes('ai_plan_quotas')) {
            const row = opts.planQuotaRow !== undefined
              ? opts.planQuotaRow
              : { quota_waku_cu: 500, description: 'Free tier — 500 WC/month' };
            return { bind: () => ({ first: async <T>() => row as T, all: async () => ({ results: [] }), run: async () => ({ success: true }) }) };
          }
          return { bind: () => ({ first: async () => null, all: async () => ({ results: [] }), run: async () => ({ success: true }) }) };
        }),
        batch: vi.fn().mockResolvedValue([{ success: true }]),
      };
    }

    it('returns 200 with quota fields', async () => {
      const app = makeApp({ dbOverride: makeQuotaDB() });
      const res = await app.request('/superagent/usage/quota');
      expect(res.status).toBe(200);
      const json = await res.json() as Record<string, unknown>;
      expect(json).toHaveProperty('plan');
      expect(json).toHaveProperty('used_waku_cu');
      expect(json).toHaveProperty('quota_waku_cu');
      expect(json).toHaveProperty('remaining_waku_cu');
      expect(json).toHaveProperty('unlimited');
      expect(json).toHaveProperty('reset_date');
    });

    it('returns correct used_waku_cu from wc_wallets', async () => {
      const db = makeQuotaDB({
        walletRow: { current_month_spent_wc: 123, spend_cap_monthly_wc: 500, spend_cap_reset_at: '2026-05-01' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { used_waku_cu: number };
      expect(json.used_waku_cu).toBe(123);
    });

    it('returns correct quota_waku_cu from ai_plan_quotas for free plan', async () => {
      const db = makeQuotaDB({
        subscriptionPlan: 'free',
        planQuotaRow: { quota_waku_cu: 500, description: 'Free tier — 500 WC/month' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { quota_waku_cu: number; plan: string };
      expect(json.plan).toBe('free');
      expect(json.quota_waku_cu).toBe(500);
    });

    it('returns correct quota_waku_cu for starter plan (5000)', async () => {
      const db = makeQuotaDB({
        subscriptionPlan: 'starter',
        planQuotaRow: { quota_waku_cu: 5000, description: 'Starter plan — 5,000 WC/month' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { quota_waku_cu: number; plan: string };
      expect(json.plan).toBe('starter');
      expect(json.quota_waku_cu).toBe(5000);
    });

    it('returns correct quota_waku_cu for professional plan (50000)', async () => {
      const db = makeQuotaDB({
        subscriptionPlan: 'professional',
        planQuotaRow: { quota_waku_cu: 50000, description: 'Professional plan — 50,000 WC/month' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { quota_waku_cu: number; plan: string };
      expect(json.plan).toBe('professional');
      expect(json.quota_waku_cu).toBe(50000);
    });

    it('calculates remaining_waku_cu = quota - used', async () => {
      const db = makeQuotaDB({
        walletRow: { current_month_spent_wc: 200, spend_cap_monthly_wc: 500, spend_cap_reset_at: '2026-05-01' },
        planQuotaRow: { quota_waku_cu: 500, description: 'Free tier' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { remaining_waku_cu: number };
      expect(json.remaining_waku_cu).toBe(300);
    });

    it('clamps remaining_waku_cu to 0 when over quota (no negative remaining)', async () => {
      const db = makeQuotaDB({
        walletRow: { current_month_spent_wc: 600, spend_cap_monthly_wc: 500, spend_cap_reset_at: '2026-05-01' },
        planQuotaRow: { quota_waku_cu: 500, description: 'Free tier' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { remaining_waku_cu: number };
      expect(json.remaining_waku_cu).toBe(0);
    });

    it('unlimited = true and remaining_waku_cu = null for enterprise (quota = 0)', async () => {
      const db = makeQuotaDB({
        subscriptionPlan: 'enterprise',
        walletRow: { current_month_spent_wc: 99999, spend_cap_monthly_wc: 0, spend_cap_reset_at: '2026-05-01' },
        planQuotaRow: { quota_waku_cu: 0, description: 'Enterprise plan — unlimited' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { unlimited: boolean; remaining_waku_cu: null; used_waku_cu: number };
      expect(json.unlimited).toBe(true);
      expect(json.remaining_waku_cu).toBeNull();
      expect(json.used_waku_cu).toBe(99999);
    });

    it('returns plan = free when no subscription row exists', async () => {
      const db = makeQuotaDB({ subscriptionPlan: null });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
    ...
    it('returns used_waku_cu = 0 when no wallet row exists yet', async () => {
      const db = makeQuotaDB({ walletRow: null });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { used_waku_cu: number };
      expect(json.used_waku_cu).toBe(0);
    });

    it('falls back to static quota (500) when ai_plan_quotas table row missing', async () => {
      const db = makeQuotaDB({ planQuotaRow: null, subscriptionPlan: 'free' });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { quota_waku_cu: number };
      expect(json.quota_waku_cu).toBe(500);
    });

    it('falls back to static quota (5000) for starter when table row missing', async () => {
      const db = makeQuotaDB({ planQuotaRow: null, subscriptionPlan: 'starter' });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { quota_waku_cu: number };
      expect(json.quota_waku_cu).toBe(5000);
    ...
    it('reset_date comes from wallet row when present', async () => {
      const db = makeQuotaDB({
        walletRow: { current_month_spent_wc: 0, spend_cap_monthly_wc: 500, spend_cap_reset_at: '2026-06-01' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { reset_date: string };
      expect(json.reset_date).toBe('2026-06-01');
    });

    it('returns 200 authenticated (auth guard covered by global middleware — tested in B1 suite)', async () => {
      const app = makeApp({ dbOverride: makeQuotaDB() });
      const res = await app.request('/superagent/usage/quota');
      // Route is authenticated — makeApp always provides auth context.
      // The 401 path is enforced by global authMiddleware in index.ts, verified in B1 suite.
      expect(res.status).toBe(200);
    });

    it('P9 invariant: quota_waku_cu and used_waku_cu are integers (no decimals)', async () => {
      const db = makeQuotaDB({
        walletRow: { current_month_spent_wc: 123, spend_cap_monthly_wc: 500, spend_cap_reset_at: '2026-05-01' },
        planQuotaRow: { quota_waku_cu: 500, description: 'Free tier' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { quota_waku_cu: number; used_waku_cu: number; remaining_waku_cu: number };
      expect(Number.isInteger(json.quota_waku_cu)).toBe(true);
      expect(Number.isInteger(json.used_waku_cu)).toBe(true);
      expect(Number.isInteger(json.remaining_waku_cu)).toBe(true);
    });

    it('includes description field from ai_plan_quotas', async () => {
      const db = makeQuotaDB({
        planQuotaRow: { quota_waku_cu: 5000, description: 'Starter plan — 5,000 WC/month (~5M input tokens)' },
      });
      const app = makeApp({ dbOverride: db });
      const res = await app.request('/superagent/usage/quota');
      const json = await res.json() as { description: string };
      expect(json.description).toContain('5,000');
    });

    it('T3 invariant: queries are scoped to caller tenant_id', async () => {
      const queriedSqlArgs: string[][] = [];
      const db = {
        prepare: vi.fn().mockImplementation((sql: string) => ({
          bind: (...args: unknown[]) => {
            queriedSqlArgs.push([sql, ...args.map(String)]);
            return {
              first: async () => null,
              all: async () => ({ results: [] }),
      ...
      const app = makeApp({ tenantId: 'tenant-abc', dbOverride: db });
      await app.request('/superagent/usage/quota');
      const walletCall = queriedSqlArgs.find(([sql]) => sql?.includes('wc_wallets'));
      expect(walletCall).toBeDefined();
      expect(walletCall).toContain('tenant-abc');
    });
  });
});
