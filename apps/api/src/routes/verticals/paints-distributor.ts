...
paintsDistributorRoutes.post('/:id/inventory', async (c) => {
  const auth = c.get('auth') as { tenantId: string }; let b: Record<string, unknown>; try { b = await c.req.json(); } catch { return c.json({ error: 'Invalid JSON body' }, 400); }
  try { return c.json({ item: await new PaintsDistributorRepository(c.env.DB).addInventory(c.req.param('id'), auth.tenantId, { productName: b['product_name'] as string, brand: b['brand'] as string | undefined, productType: b['product_type'] as string | undefined, sizeCode: b['size_code'] as string | undefined, colour: b['colour'] as string | undefined, quantityInStock: b['quantity_in_stock'] as number ?? 0, unitCostKobo: b['unit_cost_kobo'] as number, retailPriceKobo: b['retail_price_kobo'] as number, wholesalePriceKobo: b['wholesale_price_kobo'] as number | undefined, reorderLevel: b['reorder_level'] as number | undefined }) }, 201); } catch (e) { return c.json({ error: (e as Error).message }, 422); }
});
...
paintsDistributorRoutes.post('/:id/orders', async (c) => {
  const auth = c.get('auth') as { tenantId: string }; let b: Record<string, unknown>; try { b = await c.req.json(); } catch { return c.json({ error: 'Invalid JSON body' }, 400); }
  try { return c.json({ order: await new PaintsDistributorRepository(c.env.DB).createOrder(c.req.param('id'), auth.tenantId, { customerRefId: b['customer_ref_id'] as string, items: b['items'] as string, totalKobo: b['total_kobo'] as number, orderDate: b['order_date'] as number, isWholesale: b['is_wholesale'] as boolean | undefined, deliveryDate: b['delivery_date'] as number | undefined }) }, 201); } catch (e) { return c.json({ error: (e as Error).message }, 422); }
});
