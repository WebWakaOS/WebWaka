/**
 * Courier vertical route tests — P9 Set D (V-TRN-EXT-D2)
 * ≥10 cases: CRUD, FSM, T3 isolation, P9 integer kobo, riders/parcels.
 */
...
function makeApp(tenantId = 'tnt_a') {
  ...
  w.route('/', courierRoutes);
  return w;
}

const MOCK = { id: 'cur_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', companyName: 'Swift Couriers', status: 'seeded' };

describe('POST / — create courier profile', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('returns 201 for valid creation', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Swift Couriers' }) });
  ...
  it('T3: tenantId from auth JWT, not body', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_x').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Swift Couriers' }) });
...
describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/cur_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce({ ...MOCK, companyName: 'Swift Pro' });
    expect((await makeApp().request('/cur_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_name: 'Swift Pro' }) })).status).toBe(200);
  });
...
describe('POST /:id/transition — FSM', () => {
  ...
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/cur_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'active' }) })).status).toBe(422);
  });
  ...
  it('returns 200 for valid seeded→claimed transition', async () => {
    ...
    expect((await makeApp().request('/cur_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'claimed' }) })).status).toBe(200);
  });
...
describe('POST /:id/riders', () => {
  it('returns 201 for valid rider creation', async () => {
    mockRepo.createRider.mockResolvedValueOnce({ id: 'rid_001', riderName: 'Emeka Okafor', phone: '08012345678' });
    const res = await makeApp().request('/cur_001/riders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rider_name: 'Emeka Okafor', phone: '08012345678' }) });
    expect(res.status).toBe(201);
  });
});
...
describe('GET /:id/riders', () => {
  it('returns 200 with rider list', async () => {
    ...
    expect((await makeApp().request('/cur_001/riders')).status).toBe(200);
  });
});
