/**
 * Insurance Agent routes — M9
 * FSM: seeded → claimed → naicom_verified → active
 * AI: L2 cap; P13 no policy holder PII to AI
...
import {
  InsuranceAgentRepository,
  isValidInsuranceAgentTransition,
} from '@webwaka/verticals-insurance-agent';

type Auth = { userId: string; tenantId: string };
const app = new Hono<{ Bindings: Env }>();
function repo(c: { env: Env }) { return new InsuranceAgentRepository(c.env.DB); }
...
app.patch('/profiles/:id/transition', async (c) => {
  ...
  const to = body.to as Parameters<typeof isValidInsuranceAgentTransition>[1];
  const current = await repo(c).findProfileById(c.req.param('id'), tenantId);
  if (!current) return c.json({ error: 'not found' }, 404);
  if (!isValidInsuranceAgentTransition(current.status, to)) return c.json({ error: `Invalid FSM transition: ${current.status} → ${to}` }, 422);
