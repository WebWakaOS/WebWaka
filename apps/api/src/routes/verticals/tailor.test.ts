/**
 * Tailor vertical route tests — P8 Set B (V-COMM-EXT-B10)
...
function makeApp(tenantId = 'tnt_a') {
  ...
  w.route('/', tailorRoutes);
  return w;
}

const MOCK = { id: 'tl_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', businessName: 'Couture by Amara', status: 'seeded' };

describe('POST / — create tailor profile', () => {
  beforeEach(() => vi.clearAllMocks());

  it('returns 201 for valid creation', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', business_name: 'Couture by Amara' }) });
  ...
  it('T3: tenantId from auth JWT', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_q').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', business_name: 'Couture by Amara' }) });
    expect(mockRepo.createProfile).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_q' }));
  });
});
...
describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/tl_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce({ ...MOCK, businessName: 'Couture Premium' });
    expect((await makeApp().request('/tl_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ business_name: 'Couture Premium' }) })).status).toBe(200);
  });
...
describe('POST /:id/transition — FSM', () => {
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/tl_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'active' }) })).status).toBe(422);
  });
  it('returns 200 for valid transition', async () => {
    ...
    expect((await makeApp().request('/tl_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'claimed' }) })).status).toBe(200);
  });
