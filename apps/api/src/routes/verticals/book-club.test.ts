/**
 * Book Club vertical route tests — P9 Set E (V-CIV-EXT-E5)
 * ≥10 cases: CRUD, FSM, T3 isolation.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import { bookClubRoutes } from './book-club.js';

const { mockRepo, mockIsValid } = vi.hoisted(() => ({
  mockRepo: {
    create: vi.fn(), findById: vi.fn(), update: vi.fn(), transition: vi.fn(),
    createReadingRecord: vi.fn(), listReadingRecords: vi.fn(),
  },
  mockIsValid: vi.fn().mockReturnValue(true),
}));

vi.mock('@webwaka/verticals-book-club', () => ({
  BookClubRepository: vi.fn(() => mockRepo),
  isValidBookClubTransition: mockIsValid,
}));
...
function makeApp(tenantId = 'tnt_a') {
  ...
  w.route('/', bookClubRoutes);
  return w;
}

const MOCK = { id: 'bc_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', clubName: 'Eko Readers Circle', status: 'seeded' };

describe('POST / — create book club profile', () => {
  beforeEach(() => vi.clearAllMocks());

  it('returns 201 for valid creation', async () => {
    mockRepo.create.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', club_name: 'Eko Readers Circle' }) });
  ...
  it('T3: tenantId injected from auth', async () => {
    mockRepo.create.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_x').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', club_name: 'Eko Readers Circle' }) });
    expect(mockRepo.create).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_x' }));
  });
});

describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/bc_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.update.mockResolvedValueOnce({ ...MOCK, clubName: 'Eko Readers Pro' });
    expect((await makeApp().request('/bc_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ club_name: 'Eko Readers Pro' }) })).status).toBe(200);
  });
...
describe('POST /:id/transition — FSM', () => {
  ...
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/bc_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'active' }) })).status).toBe(422);
  });
  ...
  it('returns 200 for valid seeded→claimed transition', async () => {
    ...
    expect((await makeApp().request('/bc_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'claimed' }) })).status).toBe(200);
  });
