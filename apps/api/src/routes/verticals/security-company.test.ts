/**
 * Security Company vertical route tests — P8 Set B (V-COMM-EXT-B7)
 * ≥10 cases: CRUD, FSM, T3 isolation.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import { securityCompanyRoutes } from './security-company.js';
...
vi.mock('@webwaka/verticals-security-company', () => ({
  SecurityCompanyRepository: vi.fn(() => mockRepo),
  guardSeedToClaimed: vi.fn().mockReturnValue({ allowed: true }), guardClaimedToCacVerified: vi.fn().mockReturnValue({ allowed: true }), guardClaimedToNscVerified: vi.fn().mockReturnValue({ allowed: true }),
  isValidSecurityCompanyTransition: mockIsValid,
}));
...
function makeApp(tenantId = 'tnt_a') {
  ...
  w.route('/', securityCompanyRoutes);
  return w;
}

const MOCK = { id: 'sc_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', companyName: 'Shield Nigeria Security', status: 'seeded' };

describe('POST / — create security company profile', () => {
  beforeEach(() => vi.clearAllMocks());

  it('returns 201 for valid creation', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Shield Nigeria Security' }) });
  ...
  it('T3: tenantId from auth JWT', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_n').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Shield Nigeria Security' }) });
    expect(mockRepo.createProfile).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_n' }));
  });
});
...
describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/sc_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce({ ...MOCK, companyName: 'Shield Elite' });
    expect((await makeApp().request('/sc_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_name: 'Shield Elite' }) })).status).toBe(200);
  });
...
describe('POST /:id/transition — FSM', () => {
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/sc_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'active' }) })).status).toBe(422);
  });
  it('returns 200 for valid transition', async () => {
    ...
    expect((await makeApp().request('/sc_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'claimed' }) })).status).toBe(200);
  });
