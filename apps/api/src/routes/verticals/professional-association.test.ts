/**
 * Professional Association vertical route tests — P9 Set E (V-CIV-EXT-E6)
 * ≥10 cases: CRUD, FSM, T3 isolation.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import { professionalAssociationRoutes } from './professional-association.js';

const { mockRepo, mockIsValid } = vi.hoisted(() => ({
  mockRepo: {
    create: vi.fn(), findById: vi.fn(), update: vi.fn(), transition: vi.fn(),
    createMember: vi.fn(), listMembers: vi.fn(),
  },
  mockIsValid: vi.fn().mockReturnValue(true),
}));

vi.mock('@webwaka/verticals-professional-association', () => ({
  ProfessionalAssocRepository: vi.fn(() => mockRepo),
  isValidProfessionalAssocTransition: mockIsValid,
}));
...
function makeApp(tenantId = 'tnt_a') {
  ...
  w.route('/', professionalAssociationRoutes);
  return w;
}

const MOCK = { id: 'pa_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', assocName: 'Nigeria Bar Association Lagos', status: 'seeded' };

describe('POST / — create professional association profile', () => {
  beforeEach(() => vi.clearAllMocks());

  it('returns 201 for valid creation', async () => {
    mockRepo.create.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', assoc_name: 'Nigeria Bar Association Lagos' }) });
  ...
  it('T3: tenantId injected from auth', async () => {
    mockRepo.create.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_x').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', assoc_name: 'NBA Lagos' }) });
    expect(mockRepo.create).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_x' }));
  });
});

describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/pa_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.update.mockResolvedValueOnce({ ...MOCK, assocName: 'NBA Abuja' });
    expect((await makeApp().request('/pa_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ assoc_name: 'NBA Abuja' }) })).status).toBe(200);
  });
