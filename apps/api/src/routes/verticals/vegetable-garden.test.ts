/**
 * Vegetable Garden / Urban Farm vertical route tests — P9 Set G (V-AGRO-G10)
 * ≥10 cases: CRUD (/profiles), FSM, T3, P9 integer grams/sqm, plots.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import app from './vegetable-garden.js';
...
vi.mock('@webwaka/verticals-vegetable-garden', () => ({
  VegetableGardenRepository: vi.fn(() => mockRepo),
...
const MOCK = { id: 'vg_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', farmName: 'Lekki Urban Farm', status: 'seeded' };

describe('POST /profiles — create vegetable garden profile', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('returns 201 for valid creation', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/profiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: 'wsp_a', farmName: 'Lekki Urban Farm' }) });
  ...
  it('T3: tenantId from auth JWT', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_x').request('/profiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: 'wsp_a', farmName: 'Lekki Urban Farm' }) });
    expect(mockRepo.createProfile).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_x' }));
  });
});

describe('GET /profiles/:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/profiles/vg_001')).status).toBe(200); });
...
describe('PATCH /profiles/:id/transition — FSM', () => {
  ...
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/profiles/vg_001/transition', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'active' }) })).status).toBe(422);
  });
  ...
  it('returns 200 for valid seeded→claimed transition', async () => {
    ...
    expect((await makeApp().request('/profiles/vg_001/transition', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'claimed' }) })).status).toBe(200);
  });
...
describe('POST /profiles/:id/plots — P9 integer sqm', () => {
  it('returns 201 for valid plot registration', async () => {
    mockRepo.createPlot.mockResolvedValueOnce({ id: 'plt_001', plotCode: 'LF-01', areaSqm: 50, crop: 'spinach' });
    expect((await makeApp().request('/profiles/vg_001/plots', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plotCode: 'LF-01', areaSqm: 50, crop: 'spinach', plantingDate: 1700000000 }) })).status).toBe(201);
  });
});
