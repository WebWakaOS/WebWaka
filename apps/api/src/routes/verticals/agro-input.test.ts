/**
 * Agro-Input Supply vertical route tests — P9 Set G (V-AGRO-G3)
 * ≥10 cases: CRUD (/profiles), FSM, T3, P9 unit_price_kobo, catalogue.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import app from './agro-input.js';
...
const MOCK = { id: 'ai_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', companyName: 'Kebbi Agro Inputs', status: 'seeded' };

describe('POST /profiles — create agro-input profile', () => {
  beforeEach(() => vi.clearAllMocks());

  it('returns 201 for valid creation', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/profiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: 'wsp_a', companyName: 'Kebbi Agro Inputs' }) });
  ...
  it('T3: tenantId from auth JWT', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_x').request('/profiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: 'wsp_a', companyName: 'Kebbi Agro Inputs' }) });
    expect(mockRepo.createProfile).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_x' }));
  });
});

describe('GET /profiles/:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/profiles/ai_001')).status).toBe(200); });
...
describe('PATCH /profiles/:id/transition — FSM', () => {
  ...
  it('returns 422 for invalid FSM transition', async () => {
    ...
    expect((await makeApp().request('/profiles/ai_001/transition', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'active' }) })).status).toBe(422);
  });
  ...
  it('returns 200 for valid seeded→claimed transition', async () => {
    ...
    expect((await makeApp().request('/profiles/ai_001/transition', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: 'claimed' }) })).status).toBe(200);
  });
...
describe('POST /profiles/:id/catalogue — P9 unit_price_kobo', () => {
  it('returns 201 for valid catalogue item', async () => {
    mockRepo.createCatalogueItem.mockResolvedValueOnce({ id: 'cat_001', productName: 'NPK Fertiliser 50kg', unitPriceKobo: 3500000 });
    expect((await makeApp().request('/profiles/ai_001/catalogue', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productName: 'NPK Fertiliser 50kg', unit: 'bag', stockQty: 200, unitPriceKobo: 3500000 }) })).status).toBe(201);
  });
});
