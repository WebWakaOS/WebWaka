/**
 * Property Developer vertical route tests — P8 Set B (V-COMM-EXT-B4)
 * ≥10 cases: CRUD, FSM, T3 isolation.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import { propertyDeveloperRoutes } from './property-developer.js';
...
vi.mock('@webwaka/verticals-property-developer', () => ({
  PropertyDeveloperRepository: vi.fn(() => mockRepo),
  guardSeedToClaimed: vi.fn().mockReturnValue({ allowed: true }), guardClaimedToCacVerified: vi.fn().mockReturnValue({ allowed: true }),
  isValidPropertyDeveloperTransition: mockIsValid,
}));
...
function makeApp(tenantId = 'tnt_a') {
  ...
  w.route('/', propertyDeveloperRoutes);
  return w;
}

const MOCK = { id: 'pd_001', workspaceId: 'wsp_a', tenantId: 'tnt_a', companyName: 'Lekki Homes Ltd', status: 'seeded' };

describe('POST / — create property developer profile', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('returns 201 for valid creation', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Lekki Homes Ltd' }) });
  ...
  it('T3: tenantId from auth JWT', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_k').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Lekki Homes Ltd' }) });
    expect(mockRepo.createProfile).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_k' }));
  });
});
...
describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/pd_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce({ ...MOCK, companyName: 'Lekki Homes Premium' });
    expect((await makeApp().request('/pd_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_name: 'Lekki Homes Premium' }) })).status).toBe(200);
  });
...
describe('POST /:id/transition — FSM', () => {
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/pd_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'active' }) })).status).toBe(422);
  });
  it('returns 200 for valid transition', async () => {
    ...
    expect((await makeApp().request('/pd_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'claimed' }) })).status).toBe(200);
  });
