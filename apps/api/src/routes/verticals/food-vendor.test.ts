/**
 * Food Vendor vertical route tests — P8 Set A (V-COMM-EXT-A9)
 * ≥10 cases: CRUD, FSM, T3 isolation.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import { foodVendorRoutes } from './food-vendor.js';
...
vi.mock('@webwaka/verticals-food-vendor', () => ({
  FoodVendorRepository: vi.fn(() => mockRepo),
  guardSeedToClaimed: vi.fn().mockReturnValue({ allowed: true }),
  isValidFoodVendorTransition: mockIsValid,
}));
...
describe('POST / — create food vendor profile', () => {
  ...
  it('returns 201 for valid creation', async () => {
    ...
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', vendor_name: 'Iya Risi Kitchen', lga: 'Surulere', state: 'Lagos' }) });
  ...
  it('T3: tenantId from auth JWT', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_g').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', vendor_name: 'Iya Risi Kitchen', lga: 'Surulere', state: 'Lagos' }) });
    expect(mockRepo.createProfile).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tnt_g' }));
  });
});
...
describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/fv_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce({ ...MOCK, vendorName: 'Iya Risi Premium' });
    expect((await makeApp().request('/fv_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ vendor_name: 'Iya Risi Premium' }) })).status).toBe(200);
  });
  it('returns 404 when not found', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce(null);
    expect((await makeApp().request('/nx', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ vendor_name: 'X' }) })).status).toBe(404);
  });
});
...
describe('POST /:id/transition — FSM', () => {
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/fv_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'active' }) })).status).toBe(422);
  });
  it('returns 200 for valid transition', async () => {
    ...
    expect((await makeApp().request('/fv_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'claimed' }) })).status).toBe(200);
  });
