/**
 * Container Depot vertical route tests — P9 Set D (V-TRN-EXT-D6)
 * ≥10 cases: CRUD, FSM, T3 isolation, P9 daily_storage_rate_kobo, containers.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import { containerDepotRoutes } from './container-depot.js';
...
const { mockRepo, mockIsValid } = vi.hoisted(() => ({
  mockRepo: {
    ...
    createContainerRecord: vi.fn(), listContainerRecords: vi.fn(), updateContainerStatus: vi.fn(),
  },
  mockIsValid: vi.fn().mockReturnValue(true),
}));

vi.mock('@webwaka/verticals-container-depot', () => ({
  ContainerDepotRepository: vi.fn(() => mockRepo),
  guardSeedToClaimed: vi.fn().mockReturnValue({ allowed: true }),
  guardClaimedToNcsVerified: vi.fn().mockReturnValue({ allowed: true }),
  isValidContainerDepotTransition: mockIsValid,
}));
...
describe('POST / — create container depot profile', () => {
  ...
  it('returns 201 for valid creation', async () => {
    ...
    const res = await makeApp().request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Apapa Container Depot' }) });
  ...
  it('T3: tenantId from auth JWT, not body', async () => {
    mockRepo.createProfile.mockResolvedValueOnce(MOCK);
    await makeApp('tnt_x').request('/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: 'wsp_a', company_name: 'Apapa Container Depot' }) });
...
describe('GET /:id', () => {
  it('returns 200 when found', async () => { mockRepo.findProfileById.mockResolvedValueOnce(MOCK); expect((await makeApp().request('/cd_001')).status).toBe(200); });
...
describe('PATCH /:id', () => {
  it('returns 200 on update', async () => {
    mockRepo.updateProfile.mockResolvedValueOnce({ ...MOCK, companyName: 'Apapa Depot Pro' });
    expect((await makeApp().request('/cd_001', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_name: 'Apapa Depot Pro' }) })).status).toBe(200);
  });
...
describe('POST /:id/transition — FSM', () => {
  ...
  it('returns 422 for invalid transition', async () => {
    ...
    expect((await makeApp().request('/cd_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'active' }) })).status).toBe(422);
  });
  ...
  it('returns 200 for valid seeded→claimed transition', async () => {
    ...
    expect((await makeApp().request('/cd_001/transition', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to_status: 'claimed' }) })).status).toBe(200);
  });
...
describe('POST /:id/containers — P9 daily_storage_rate_kobo', () => {
  it('returns 201 for valid container record', async () => {
    mockRepo.createContainerRecord.mockResolvedValueOnce({ id: 'con_001', containerNumber: 'APZU1234567', dailyStorageRateKobo: 50000 });
    const res = await makeApp().request('/cd_001/containers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ container_number: 'APZU1234567', container_type: '20ft', weight_kg: 20000, daily_storage_rate_kobo: 50000, days_in_depot: 5 }) });
    expect(res.status).toBe(201);
  });
});

describe('GET /:id/containers', () => {
  it('returns 200 with container list', async () => {
    mockRepo.listContainerRecords.mockResolvedValueOnce([]);
    expect((await makeApp().request('/cd_001/containers')).status).toBe(200);
  });
});
