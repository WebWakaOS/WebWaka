...
export function registerMiddleware(app: Hono<{ Bindings: Env }>): void {
  ...
  // PERF-06 NOTE: Application-level gzip is DISABLED for Cloudflare Workers.
  // CF edge automatically compresses eligible responses (JSON > 1KB) via Brotli/gzip.
  // Enabling Hono compress() middleware causes double-encoding (Worker gzip + CF gzip)
  // which breaks most HTTP clients. CF edge compression is sufficient.
