-- Migration: 0415_template_registry_rejected_status
-- Pillar 2 / Emergent Pillar-2 audit fix (2026-04-25).
--
-- The CHECK constraint on template_registry.status created in 0206 only
-- allowed ('draft','pending_review','approved','deprecated'). The new
-- approval workflow API also produces 'rejected' transitions — this migration
-- relaxes the CHECK to include 'rejected'. SQLite does not support ALTER
-- COLUMN CHECK directly, so we recreate the constraint via the documented
-- table-rebuild pattern.

CREATE TABLE template_registry_new (
  id TEXT PRIMARY KEY,
  ...
  status TEXT NOT NULL DEFAULT 'draft' CHECK(status IN ('draft','pending_review','approved','deprecated','rejected')),
...
INSERT INTO template_registry_new
...
ALTER TABLE template_registry_new RENAME TO template_registry;
