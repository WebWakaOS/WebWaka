/**
 * @webwaka/verticals-gym — test suite (canonical)
 * Covers: FSM, P9 guards, T3 isolation, health-data AI guard (P13)
 * Merged from @webwaka/verticals-gym-fitness per M1 decision.
 */
import { describe, it, expect, beforeEach } from 'vitest';
import { GymRepository } from './gym.js';
import {
...
describe('@webwaka/verticals-gym (canonical)', () => {
  it('exports correct slug', () => {
    expect(VERTICAL_SLUG).toBe('gym');
  });

  it('isValidGymTransition and isValidGymFitnessTransition are the same function', () => {
    expect(isValidGymTransition).toBe(isValidGymFitnessTransition);
  });
});

describe('FSM transitions', () => {
  it('seeded → claimed is valid', () => { expect(isValidGymTransition('seeded', 'claimed')).toBe(true); });
  it('claimed → cac_verified is valid', () => { expect(isValidGymTransition('claimed', 'cac_verified')).toBe(true); });
  it('cac_verified → active is valid', () => { expect(isValidGymTransition('cac_verified', 'active')).toBe(true); });
  it('active → suspended is valid', () => { expect(isValidGymTransition('active', 'suspended')).toBe(true); });
  it('suspended → active is valid (recovery)', () => { expect(isValidGymTransition('suspended', 'active')).toBe(true); });
  it('seeded → active is invalid', () => { expect(isValidGymTransition('seeded', 'active')).toBe(false); });
  it('active → seeded is invalid', () => { expect(isValidGymTransition('active', 'seeded')).toBe(false); });
});
...
describe('AI guards', () => {
  ...
  it('guardL2AiCap passes for level 2', () => {
    expect(guardL2AiCap({ autonomyLevel: 2 }).allowed).toBe(true);
  });
  it('guardL2AiCap passes when autonomyLevel undefined', () => {
    expect(guardL2AiCap({}).allowed).toBe(true);
  });
  it('guardNoHealthMetricsToAi blocks health metrics (P13)', () => {
    expect(guardNoHealthMetricsToAi({ includesHealthMetrics: true }).allowed).toBe(false);
  });
...
describe('GymRepository', () => {
  ...
  beforeEach(() => { repo = new GymRepository(makeDb() as never); });
