/**
 * @webwaka/verticals-paints-distributor — test suite (M10)
 * Covers: FSM, L2 AI cap, T3 isolation
 */
import { describe, it, expect, beforeEach } from 'vitest';
import { PaintsDistributorRepository } from './paints-distributor.js';
import { isValidPaintsDistributorTransition, guardL2AiCap } from './types.js';
...
describe('verticals-paints-distributor', () => {
  let repo: PaintsDistributorRepository;
  beforeEach(() => { repo = new PaintsDistributorRepository(makeDb() as never); });

  it('FSM: seeded → claimed is valid', () => { expect(isValidPaintsDistributorTransition('seeded', 'claimed')).toBe(true); });
  it('FSM: claimed → son_verified is valid', () => { expect(isValidPaintsDistributorTransition('claimed', 'son_verified')).toBe(true); });
  it('FSM: son_verified → active is valid', () => { expect(isValidPaintsDistributorTransition('son_verified', 'active')).toBe(true); });
  it('FSM: active → suspended is valid', () => { expect(isValidPaintsDistributorTransition('active', 'suspended')).toBe(true); });
  it('FSM: suspended → active is valid', () => { expect(isValidPaintsDistributorTransition('suspended', 'active')).toBe(true); });
  it('FSM: seeded → active is invalid', () => { expect(isValidPaintsDistributorTransition('seeded', 'active')).toBe(false); });
  it('FSM: active → claimed is invalid', () => { expect(isValidPaintsDistributorTransition('active', 'claimed')).toBe(false); });
  ...
  it('createProfile sets status seeded', async () => {
    const p = await repo.createProfile({ workspaceId: 'ws1', tenantId: 'tn1', companyName: 'Crown Paints Ikeja' });
    expect(p.status).toBe('seeded');
    expect(p.tenantId).toBe('tn1');
  });
  it('findProfileById T3 isolation', async () => {
    const p = await repo.createProfile({ workspaceId: 'ws1', tenantId: 'tn1', companyName: 'Berger Distributors Lagos' });
    expect(await repo.findProfileById(p.id, 'tn-other')).toBeNull();
  });
  it('findProfileById returns profile for correct tenant', async () => {
    const p = await repo.createProfile({ workspaceId: 'ws1', tenantId: 'tn-pd', companyName: 'Dulux Enugu Depot' });
    const found = await repo.findProfileById(p.id, 'tn-pd');
    expect(found).not.toBeNull();
    expect(found?.tenantId).toBe('tn-pd');
  });
  it('transitionStatus changes FSM state', async () => {
    const p = await repo.createProfile({ workspaceId: 'ws1', tenantId: 'tn1', companyName: 'Sandtex Kano' });
