...
**PAYSTACK_SECRET_KEY: PENDING — must be set via `wrangler secret put PAYSTACK_SECRET_KEY --env production` in apps/api/ once the Paystack key is available. PLATFORM IS IN BANK TRANSFER MODE — Paystack key is optional while manual mode is active.**
**Bank Transfer Default Payment Mode: LIVE 2026-04-22. DEFAULT_PAYMENT_MODE="bank_transfer" set in wrangler.toml (staging + production). PLATFORM_BANK_ACCOUNT_JSON set with placeholder account details — UPDATE before going live. POST /workspaces/:id/upgrade + POST /workspaces/:id/activate now return bank transfer instructions (reference, narration, bank_account, instructions, expires_at) instead of 503 when no Paystack key. GET /payments/method endpoint added (auth required) — returns payment_mode, gateway_available, plan amounts, and bank_account when in manual mode. Wallet KV flags: transfers_enabled=1, withdrawals_enabled=1, online_funding_enabled=0 in production WALLET_KV (namespace e28f499f). PAYSTACK_SECRET_KEY made optional in Env interface; IdentityEnv also updated (Paystack BVN fallback only used when Prembly fails). packages/identity/src/bvn.ts: throws informative IdentityError when Paystack fallback needed but key absent.**
**Configurable Bank Account Settings — IMPLEMENTED 2026-04-22. TWO-LEVEL bank account system: (1) Platform bank account (WebWaka's receiving account for subscription fees) — super admin sets via PATCH /platform-admin/settings/payment, stored in WALLET_KV as platform:payment:bank_account; GET /platform-admin/settings/payment reads it back. (2) Workspace bank account (each partner's own business bank account for customer payments) — workspace admin sets via PATCH /workspaces/:id {bankAccount:...}, stored in workspaces.payment_bank_account_json (migration 0378). Priority chain: WALLET_KV → PLATFORM_BANK_ACCOUNT_JSON env → default. Migrations: 0378 (workspace bank account column), 0379 (tenant_branding domain verification + bank account), 0380 (claim_requests.workspace_id).**
**Claim → Activation Flow Gaps Fixed — 2026-04-22. (1) Tenant branding API added: PATCH /tenant/branding creates/updates tenant_branding table (colors, logo, fonts, custom domain, bank account). tenant_branding table existed since migration 0197 but had NO write endpoint — now fixed. (2) Custom domain verification workflow: POST /tenant/branding/domain registers domain + generates TXT token; GET /tenant/branding/domain/verify does live DNS TXT check via Cloudflare DNS-over-HTTPS. (3) Profile visibility control: PATCH /profiles/:id/visibility lets workspace admins set public/semi/private — changes are synced to search_entries to keep discovery indexes current. (4) Active layers persist bug fixed: PATCH /workspaces/:id now actually writes active_layers to DB (was firing the event but the SQL UPDATE was missing). (5) Claim → workspace link: claim_requests now stores workspace_id (migration 0380); POST /claim/advance (approve) links profiles.workspace_id to the approving workspace. (6) Profile managed state: PATCH /profiles/:id/claim-state allows workspace owner on paid plan to advance profile from verified → managed. New route files: apps/api/src/routes/platform-admin-settings.ts, apps/api/src/routes/tenant-branding.ts, apps/api/src/routes/profiles.ts.**
...
### Round 5 Audit Bug Fixes (2026-04-23)

Fourteen parallel audit agents identified ~30 bugs; all confirmed issues were fixed with zero new test regressions (same 5 pre-existing pos failures; all other tests pass).

1. **SEC-SQLI-01 — SQL injection in 17 vertical `transitionStatus` methods** (all 17 affected verticals): Every `transitionStatus` method built SQL using string interpolation, e.g. `` `, col='${fields.cacRc}'` ``. An attacker controlling those fields could inject arbitrary SQL. Fix: replaced with `extraClauses: string[]` + `extraBinds: unknown[]` arrays built via `push()`, with the values bound via `.bind(to, ...extraBinds, id, tenantId)`. Affected packages: `verticals-furniture-maker`, `verticals-gas-distributor`, `verticals-generator-repair`, `verticals-govt-school`, `verticals-handyman`, `verticals-hotel`, `verticals-internet-cafe`, `verticals-it-support`, `verticals-land-surveyor`, `verticals-logistics-delivery`, `verticals-ministry-mission`, `verticals-oil-gas-services`, `verticals-okada-keke`, `verticals-optician`, `verticals-orphanage`, `verticals-pharmacy-chain`, `verticals-savings-group`.

2. **SEC-HTML-01 — HTML injection in email templates** (`apps/api/src/lib/email-service.ts`): All 6 transactional email templates interpolated user-provided strings (`name`, `workspace_name`, `buyer_name`, `inviter_name`, `payer_name`, `plan_name`, `template_name`, `transaction_ref`, `purchase_date`, `payment_date`) directly into HTML without escaping. An attacker controlling their display name could inject arbitrary HTML into emails. Fix: added `escapeHtml(str)` helper (replaces `&`, `<`, `>`, `"`, `'`) and applied it to every user-controlled string in all 6 templates.

3. **T3-RESET-01 — password reset UPDATE not tenant-scoped** (`apps/api/src/routes/auth-routes.ts`): `POST /auth/reset-password` ran `UPDATE users SET password_hash … WHERE id = ?` with no `tenant_id` guard — a T3 violation. Fix: added a pre-UPDATE `SELECT tenant_id FROM users WHERE id = ?` lookup; both the `UPDATE users` and `DELETE FROM sessions` are now scoped with `AND tenant_id = ?`.

4. **T3-VERIFY-01 — verify-email UPDATE not tenant-scoped** (`apps/api/src/routes/auth-routes.ts`): `POST /auth/verify-email` ran `UPDATE users SET email_verified_at … WHERE id = ? AND email_verified_at IS NULL` with no `tenant_id`. Fix: same pattern — lookup tenant_id first, then apply `AND tenant_id = ?` to the UPDATE.

5. **SEC-INVITE-02 — accept-invite includes revoked invitations** (`apps/api/src/routes/auth-routes.ts`): The invitation SELECT at `/accept-invite` did not filter out revoked invitations (`revoked_at IS NULL`). A revoked invite token could still be redeemed. Fix: added `AND revoked_at IS NULL` to the invitation lookup query.

6. **BUG-WALLET-01 — creditWallet/debitWallet balance read TOCTOU** (`packages/hl-wallet/src/ledger.ts`): Both functions ran two separate D1 round-trips: `UPDATE hl_wallets … WHERE …` followed by `SELECT balance_kobo …`. A concurrent write between the two statements would cause the ledger entry's `balance_after` to be stale. Fix: replaced both with `UPDATE … RETURNING balance_kobo` using `.first<{ balance_kobo: number }>()` — the balance is returned atomically in the same statement. Test mock updated to handle `RETURNING`-style `first()` calls for credit and debit paths.

7. **BUG-B2B-01 — bid-accept TOCTOU race** (`apps/api/src/routes/b2b-marketplace.ts`): `/rfqs/:rfqId/bids/:bidId/accept` read `rfq.status` in JavaScript, then issued separate INSERT (PO) and UPDATE (rfq awarded) statements. Two concurrent requests could both read `status = 'bidding'` and both create a PO. Fix: moved the `UPDATE b2b_rfqs SET status = 'awarded' … WHERE … AND status = 'bidding'` before the INSERT, so the second concurrent request hits `meta.changes = 0` and returns 409.

8. **BUG-B2B-02 — invoice line items not validated** (`apps/api/src/routes/b2b-marketplace.ts`): `POST /invoices` only validated the final total; individual line items could have `total_kobo ≠ quantity × unit_price_kobo`. Fix: added per-item validation loop before the subtotal reduction: checks integer constraints on each field and verifies `item.total_kobo === item.quantity * item.unit_price_kobo`, returning HTTP 422 with a precise message on mismatch.

9. **BUG-BILLING-01 — billing enforce loop misses status guard on past_due transition** (`apps/api/src/routes/billing.ts`): `UPDATE subscriptions SET status = 'past_due' … WHERE id = ? AND tenant_id = ?` had no guard on current status — a subscription already in `grace_period` or `suspended` could be re-stamped `past_due` by a repeated cron run. Fix: added `AND status = 'active'` to the WHERE clause.

10. **BUG-BILLING-02 — billing enforce loop misses enforcement_status guard on suspend transition** (`apps/api/src/routes/billing.ts`): `UPDATE subscriptions SET status = 'suspended' … WHERE id = ? AND tenant_id = ?` had no guard. Fix: added `AND enforcement_status = 'grace_period'` to prevent double-suspending.

11. **BUG-INBOX-01 — inbox keyset cursor has tie-breaking gap** (`apps/api/src/routes/inbox-routes.ts`): The pagination cursor was a plain `created_at` integer. Multiple items with identical timestamps would be silently skipped on page boundaries. Fix: replaced with a base64-encoded composite cursor `{ t: created_at, i: id }`, with the SQL predicate `(created_at < ? OR (created_at = ? AND id < ?))`. `nextCursor` generation updated to `btoa(JSON.stringify({ t, i }))`.

12. **BUG-NGO-01 — ngo.ts `createFunding` called with empty workspaceId** (`apps/api/src/routes/verticals/ngo.ts`): The POST `/profiles/:id/funding` handler passed `workspaceId: ''` hardcoded. Fix: added a `findById` lookup on the profile first; if not found returns 404; otherwise uses `profile.workspaceId`.

13. **BUG-NGO-02 — ngo.ts missing try-catch around `req.json()`** (`apps/api/src/routes/verticals/ngo.ts`): No route wrapped `c.req.json()` in try-catch; a malformed body would produce an unhandled exception. Fix: all JSON parse calls are now wrapped with `try { … } catch { return 400 }`.

**Final test counts**: all packages pass; only 5 pre-existing `pos/float-ledger.test.ts` failures remain (pre-date Round 5 and are unrelated to these changes).
